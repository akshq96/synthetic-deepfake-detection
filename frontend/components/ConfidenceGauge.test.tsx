...
describe("ConfidenceGauge", () => {
  it("renders the Deepfake verdict and confidence for a fake prediction", () => {
    ...
    // Both sides of the split confidence bar are always shown, regardless of verdict.
    expect(screen.getByText("91.3% Deepfake")).toBeInTheDocument();
    expect(screen.getByText("8.7% Real")).toBeInTheDocument();
  });
  ...
  it("renders the Real verdict for a real prediction", () => {
    ...
    expect(screen.getByText("13.0% Deepfake")).toBeInTheDocument();
  });
